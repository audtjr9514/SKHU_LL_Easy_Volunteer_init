...
class CUser(AbstractUser):
    codeNum = models.IntegerField(blank=True, null=True)     # 주민 번호
    phoneNum = models.IntegerField(blank=True, null=True)    # 핸드폰 번호
    job = models.ForeignKey('Job', on_delete=models.CASCADE,
                            related_name='user_job', blank=True, null=True)  # 직업
    license = models.CharField(max_length=20, blank=True, null=True)   # 자격증
    level = models.IntegerField(default=1)      # 레벨
    point = models.IntegerField(default=0)      # 가지고 있는포인트
    area = models.ForeignKey(
        'Area', on_delete=models.CASCADE, related_name='user_area', blank=True, null=True)  # 지역
    another = models.CharField(max_length=100, blank=True, null=True)  # 비고
    image = models.ImageField(
        upload_to='images/', blank=True, null=True)  # 이미지
    is_organ = models.BooleanField(default=False)   # 일반 회원인지, 기관 회원인지 확인
    organ = models.OneToOneField(
        'Organ', on_delete=models.CASCADE, related_name='user_organ', blank=True, null=True)  # 기관에 대한 정보
    def __str(self):
...
class Organ(models.Model):
    ...
    area = models.ForeignKey(
        'Area', on_delete=models.CASCADE, related_name='organ_area')  # 지역
    ...
    def __str(self):
        return self.GenralUser.username
